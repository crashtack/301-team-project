  ...
  Permit.getData = function() {
    webDB.execute('SELECT * FROM permitdata', function(rows) {
      if (rows.length) {
        ...
        // $.get('https://data.seattle.gov/resource/94s7-sxg7.json?$$app_token=gdkMQ6LU9xq4ZonjF6aDFun5l&$limit=1000&permit_type=Construction&action_type=NEW', function(data) {
        $.get('https://data.seattle.gov/resource/94s7-sxg7.json?$$app_token=gdkMQ6LU9xq4ZonjF6aDFun5l&$limit=50000&permit_type=Construction&action_type=NEW&$where=NOT%20status=%27Permit%20Closed%27', function(data) {
        // console.log(data);
